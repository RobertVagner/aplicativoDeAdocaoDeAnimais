...
class AdotePataPataSupabaseUser extends BaseAuthUser {
  AdotePataPataSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> adotePataPataSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AdotePataPataSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
